...
int	CONFIG_POLLER_WEB_TIMEOUT	= 10;
...
static void	zbx_load_config()
{
	static struct cfg_line	cfg[] =
	{
		...
		{"StartPollers",		&CONFIG_POLLER_FORKS,			TYPE_INT,
			PARM_OPT,	0,			1000},
		{"StartPollersWeb",		&CONFIG_POLLER_WEB_FORKS,		TYPE_INT,
			PARM_OPT,	0,			1000},
		...
		{"TrapperTimeout",		&CONFIG_TRAPPER_TIMEOUT,		TYPE_INT,
			PARM_OPT,	1,			300},
		{"PollerWebTimeout",		&CONFIG_POLLER_WEB_TIMEOUT,		TYPE_INT,
			PARM_OPT,	2,			99},
...
#	define IPV6_FEATURE_STATUS	" NO"
	...
	zabbix_log(LOG_LEVEL_INFORMATION, "WEB check monitoring:      " LIBCURL_FEATURE_STATUS);
	...
	threads_num = CONFIG_CONFSYNCER_FORKS + CONFIG_WATCHDOG_FORKS + CONFIG_POLLER_FORKS
			...
			+ CONFIG_SELFMON_FORKS + CONFIG_VMWARE_FORKS + CONFIG_POLLER_WEB_FORKS;
	...
	else if (server_num <= (server_count += CONFIG_POLLER_WEB_FORKS))
	{
		INIT_SERVER(ZBX_PROCESS_TYPE_WEB, CONFIG_POLLER_WEB_FORKS);

		main_poller_loop(ZBX_POLLER_TYPE_WEB);
	}
